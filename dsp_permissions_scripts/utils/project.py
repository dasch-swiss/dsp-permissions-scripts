...
def _get_onto_iris_of_project(project_iri: str) -> list[str]:
    try:
        response = dsp_client.dspClient.get("/v2/ontologies/metadata")
...
def _get_class_iris_of_onto(onto_iri: str) -> list[str]:
    try:
        response = dsp_client.dspClient.get(f"/v2/ontologies/allentities/{quote_plus(onto_iri)}")
    except ApiError as err:
...
def get_all_resource_class_iris_of_project(project_iri: str) -> list[str]:
    logger.info(f"Getting all resource class IRIs of project {project_iri}...")
    project_onto_iris = _get_onto_iris_of_project(project_iri)
    all_class_iris = []
    for onto_iri in project_onto_iris:
        class_iris = _get_class_iris_of_onto(onto_iri)
...
def get_project_iri_by_shortcode(shortcode: str) -> str:
    try:
        response = dsp_client.dspClient.get(f"/admin/projects/shortcode/{shortcode}")
    except ApiError as err:
