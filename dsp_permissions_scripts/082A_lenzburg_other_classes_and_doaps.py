...
def modify_doaps(doaps: list[Doap]) -> list[Doap]:
    ...
    for doap in copy.deepcopy(doaps):
        if isinstance(doap.target, GroupDoapTarget) and doap.target.group == group.PROJECT_MEMBER:
            doap.scope = PUBLIC
...
def update_doaps(shortcode: str, dsp_client: DspClient) -> None:
    ...
    _ = create_new_doap_on_server(
        target=NewEntityDoapTarget(prefixed_class="sva:AudioRepresentation"),
        shortcode=shortcode,
        scope=LIMITED_VIEW,
        dsp_client=dsp_client,
    )
...
def main() -> None:
    ...
    oap_config = OapRetrieveConfig(
        ...
        specified_res_classes=["sva:Agent", "sva:Collection", "sva:CSVFile", "sva:Object"],
        retrieve_values="all",
    )
